...
function priceCalculation(){
  ...
  let subprice = addedItems.map((item)=> item.price * item.amount);
  
  let subprice1 = subprice.reduce((accumulator, currentValue) => accumulator + currentValue);
...
function App() {
  ...
  console.log(temp.map((item,index) => amountTemp.push(item[index].amount)));
  
  let totalAmount = amountTemp.reduce(
